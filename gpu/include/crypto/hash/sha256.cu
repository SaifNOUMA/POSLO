...
__device__ __forceinline__ static int d_SHA256_Init(self_SHA256_CTX *c)
{
	memset(c, 0, sizeof(*c));
	c->h[0] = 0x6a09e667UL;
	c->h[1] = 0xbb67ae85UL;
...
__device__ __forceinline__  static void sha256_block_data_order(self_SHA256_CTX *ctx, const void *in,
					size_t num)
{
...
__device__ __forceinline__  static int d_SHA256_Update(self_SHA256_CTX *c, const void *data_, size_t len)
{
...
__device__  __forceinline__ static int d_SHA256_Final(u8 *md, self_SHA256_CTX *c)
{
	u8 *p = (u8 *)c->data;
	size_t n = c->num;
				       ...
				       size_t size, size_t n)
{
	...
	if (tid < n) {
		d_SHA256_Init(&c);
		d_SHA256_Update(&c, msg + tid * size, size);
		d_SHA256_Final(m, &c);
...
__device__ __forceinline__ void d_sha256_i(const u8* in, u32 inlen, u8* hash, u32 counter)
{
    ...
    memcpy(prior_hash + inlen, (u8*) &counter, sizeof(u32));
	
	d_SHA256(prior_hash, inlen + sizeof(u32), hash);
}
